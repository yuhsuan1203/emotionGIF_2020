...
from pybert.io.roberta_processor import RobertaProcessor
from pybert.common.tools import init_logger, logger
from pybert.common.tools import seed_everything
from pybert.configs.basic_config import config
from pybert.model.roberta_for_multi_label import RobertaForMultiLable
...
from pybert.callback.optimizer.adamw import AdamW
...
def run_test(args):
    ...
    test_dataloader = DataLoader(test_dataset, sampler=test_sampler, batch_size=args.train_batch_size,
                                 collate_fn=collate_fn)
    model = RobertaForMultiLable.from_pretrained(config['checkpoint_dir'], num_labels=len(label_list))
...
def main():
    ...
    parser.add_argument("--arch", default='roberta', type=str)  
    ...
    parser.add_argument('--data_name', default='emotionGIF', type=str)
