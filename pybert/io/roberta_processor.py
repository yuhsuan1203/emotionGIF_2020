...
class RobertaProcessor(object):
    """Base class for data converters for sequence classification data sets."""

    def __init__(self, model_type="roberta-base"): #vocab_path, merge_path):
        self.tokenizer = RobertaTokenizer.from_pretrained(model_type) #(vocab_path, merge_path)
    ...
    def create_features(self,examples,max_seq_len,cached_features_file):
        ...
        if cached_features_file.exists():
            ...
            features = []
            # unwanted_tokens = ["_", "::", "Ġ:"] #, ":"]
            # tokens_a = [token for token in tokens_a if token not in unwanted_tokens]
            for ex_id, example in enumerate(examples):
                ...
                if example.text_b:
                    ...
                    if len(tokens_a) > max_seq_len - 2:
                        #tokens_a = tokens_a[0:128] + tokens_a[-382:]
                        tokens_a = tokens_a[:max_seq_len - 2] #2 - max_seq_len
